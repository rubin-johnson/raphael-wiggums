...
async def _run_with_mock(state, plan, repo, mock_launch_fn, max_concurrent=3):
    sup = Supervisor(state, plan, repo, max_concurrent=max_concurrent, max_retries=3)
...
async def test_supervisor_runs_dependency_free_story_first(tmp_path):
    ...
    async def mock_launch(story_id, story, attempt):
        ...
        return AgentResult(
            ...
            stderr="", exit_code=0, worktree_path="", branch=f"{story_id}-attempt-1",
        )
...
""")
    ...
    async def mock_launch(story_id, story, attempt):
        ...
        if call_count[0] == 1:
            return AgentResult(
                story_id=story_id,
                stdout=f"STORY_RETRY_NEEDED: {story_id}\nDid foo() but bar() not done.",
                stderr="", exit_code=0, worktree_path="", branch=f"{story_id}-attempt-1",
            )
        return AgentResult(
            story_id=story_id, stdout=f"STORY_COMPLETE: {story_id}",
            stderr="", exit_code=0, worktree_path="", branch=f"{story_id}-attempt-2",
        )
...
async def test_supervisor_marks_failed_after_retry_limit(tmp_path):
    state, plan = make_plan(tmp_path, """
## STORY-001 — Foo
...
""")
    ...
    async def mock_launch(story_id, story, attempt):
        return AgentResult(
            story_id=story_id,
            stdout=f"STORY_RETRY_NEEDED: {story_id}\nStill failing.",
            stderr="", exit_code=0, worktree_path="", branch=f"{story_id}-attempt-{attempt}",
        )

    sup = Supervisor(state, plan, repo, max_concurrent=3, max_retries=2)
